...
| Blank nametable row cache (MD planes) | Rig draw bucket 968,677→775,094 (**−19.98%** of draw, −1.82% of frame); **device 24.017→23.66×3 = −1.49%** — a memory-traffic lever that loses on *both* sides (unlike the sound HLE, which the rig oversold but the device kept). Hash-gated bit-identical throughout. 16 slots rejected twice over: measured *worse* (+8.5K draw, cold 192 B table costs more than the misses it saves — probe showed 93.6% hit at 4 slots already) and physically impossible (44 B BSS headroom left; 16 slots need +144 B). The residual 395 K is non-blank rows (the HUD) — the cache's rightful ceiling. VSRam strip variants not cacheable (row key shifts per cell). Shipped in 23ea0b8d + 52f47f0f; rollback/default-off is an open orchestrator call | rig `rowbase`/`rowc*`, arms `hle4`/`rowc2` (card md5 5ba4732b verified) |
