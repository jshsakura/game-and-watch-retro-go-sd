...
| Blank nametable row cache (MD planes) | Rig draw bucket 968,677→775,094 (**−19.98%** of draw, −1.82% of frame); **device 24.01×3 → 23.66×3 = −1.46% regression** (same-session A/B, `hle4` re-benched right after `rowc2`; yesterday's 24.02 held at 24.01 today, so the delta is not session drift). Hash-gated bit-identical throughout. 16 slots rejected twice over: measured *worse* (+8.5K draw, cold 192 B table costs more than the misses it saves — probe showed 93.6% hit at 4 slots already) and physically impossible (44 B BSS headroom left; 16 slots need +144 B). The residual 395 K is non-blank rows (the HUD) — the cache's rightful ceiling. VSRam strip variants not cacheable (row key shifts per cell). **Reverted** (picodrive `dc0e0b7f` + `48d3f5b6`; binary proof: `gnw_rowcache` symbols 0 after revert, rv1≠hle4 md5 is entirely 1.1's later interpreter/compositor work, not missed revert). **Lesson, now a rule: rig insn/frame reduction and device fps do not even share a sign — this lever predicted −1.82% on the rig and cost −1.46% on the device, the second memory-traffic lever the rig oversold (sound HLE: rig −9.6%/device +0.2%) and the first to cross the sign. No 32X lever ships on rig numbers alone; a device drawn bench is mandatory** | rig `rowbase`/`rowc*`/`rvg`, arms `hle4`/`rowc2`/`rv1` (card md5s 5ba4732b / b3769a89 verified) |

| Removing the compositor's solid-run detector | Rig said **-0.48%** (a gain: on QEMU the detector's failed 4-pixel probes are pure tax and its `u32` blasts are just instructions). Device said **-2.95%** -- 24.71 vs 25.46 drawn fps, arm `nrd1` vs `disp1`, same session, card md5s `729639b1` vs `70f8112c`. **The detector stays.** Second sign reversal in two days, and the sharper of the pair, because replacing per-pixel `u16` stores with `u32` blasts is exactly what a machine with no cache model cannot price | rig `nrd`, arm `nrd1`, 2026-08-20 |
| NOP short-circuit ahead of `op0000`'s second-level switch | NOP is **7.8% of every msh2 instruction** (opcode census `RIG_OPHIST`: `0x00xx` is 12.08% of dispatches, 64.7% of it NOP, 28.0% RTS) and has an empty body, so it looked free to catch. Measured **0.10%** = noise. gcc's jump table for that switch is already cheap; what a NOP costs is the loop *iteration*, and removing that costs more in the delay-slot path than it saves | rig arm B1, 2026-08-20 |
| Carrying `delay` / `test_irq` / `ppc` in interpreter locals | **+3.14%, a regression.** The dispatch loop is at its register ceiling; one more live value and gcc spills the opcode to the stack. Do not add locals to this loop -- fewer memory operations on paper, more in the object file | rig arm A5, 2026-08-20 |
| Switching off the MD sound chips for a 32X title | `-DRIG_NO_FM` -3.54%, `-DRIG_NO_Z80` -4.30% on the rig -- and **both move the audio hash**, so Doom 32X really does drive the YM2612 and the Z80. There is no idle chip here to stop emulating | rig, 2026-08-20 |
| D32XR as a way out | The official 5 MiB build now boots and renders (picodrive `7e04cb69` wired SSF2 bank writes into GNW builds; rig GATE3 PASS at 500 frames). It is **not faster, it is 3.2x heavier**: 31.78 M host insn and 319,009 guest SH-2 insn per frame against retail Doom's 9.88 M / 94,571. The **41.4 drawn fps recorded 2026-08-15 cannot be reconciled with that** -- and this tree's own discriminator says what a high frame rate over a static 3D scene means. Prove the picture moves with two screenshots before quoting a D32XR number again | rig, 2026-08-20 |

| Eight-pixel composite path (octa) | Built, hash-gated, **rig -1.02%** -- and it does not fit. Its ~150 B body is instantiated once per `make_do_loop` variant inside `PicoDraw32xLayer`, which the linker script pins in RAM_EMU, for **592 B** against a measured **12 B** of headroom (`_OVERLAY_MD32X_BSS_END = 0x240ffff4`, ELF-read; the 44 B in the blank-row row above is stale). **Shelved on budget, not on merit** -- code and `-DGNW_PP_NO_OCTA` kept, default off so a flagless build links. The bytes exist in one place: the solid-run detector occupies 2,604 B of the same region, and its three near-identical blast blocks would fund this several times over if folded into one helper (which needs its own device A/B, being the same store traffic) | picodrive `2bc61933`, 2026-08-20 |

| **The device itself drifts** | `disp1`, the same binary and the same card (md5 `70f8112c` verified off the device both times), measured **25.46/25.46/25.44 at 08:12** and **24.95/24.95/24.95 at 09:30**: **-2.0% in eighty minutes**, with zero spread at the second reading. Bigger than most levers argued about here. It nearly produced a wrong conclusion the same day -- `base2` came in 1.3% under `disp1` and looked like proof that 32 bytes of dead-code layout move the frame rate; re-benched, the two agree. **Compare arms adjacent in time, and re-bench the reference at the end of every session.** A tight spread proves the binary is reproducible, not that the device is where it was an hour ago | arms `disp1` (twice), `base2`, 2026-08-20 |
| 68K idle fold (the axis that was NOT closed) | Shipped: **+5.68% on the device** (`idl2` 26.53/26.54/26.54 against `base2` 25.12/25.11/25.09, twelve minutes apart, one build flag between them). The rig had it at **-2.74%** instruction reduction -- underweighted, as predicted before the bench: the 68K is 3.5% of the rig frame and 12.9% of the device's. Doom spends **75.2%** of its 68K instructions in one `tst.b $ff1134 / beq -8` vblank spin that picodrive's own idle whitelist already describes and that only Cyclone and FAME were ever wired to | picodrive `2a28aed8`, `aec66773` |
