...
| `test_core_contract_wired.sh` | Every core calls the four things the shared machinery needs: `odroid_system_init` (launch and the config slot), `odroid_system_emu_init` (savestates), `common_emu_frame_loop` (pacing, frameskip, FPS) and an audio start. Forgetting one removes a feature in silence, which is how Super Metroid shipped three releases with no savestates and no pacing. Exemptions carry a reason in `tests/core_contract_exempt.txt`; the audio entry points are derived from `gw_audio.c`, after a hardcoded name made the gate accuse a healthy core. |
