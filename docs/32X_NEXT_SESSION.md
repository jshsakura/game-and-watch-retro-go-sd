...
| Retail Doom, fps axis 08-19 | **17.49 (v23) → 21.39 (five levers, arm `ahle`) → 24.02 (sound HLE, arm `hle4`) = +37% cumulative**; blank-row cache on top measured **23.66×3 = −1.49%** (arm `rowc2`, card md5 5ba4732b verified) vs rig −1.82% — a memory-traffic lever that loses on both sides; rollback/default-off is an open call (see `32X_CLOSED.md` Ledger) |
