...
> 수단으로는 도달할 수 없습니다.** 기기 실측: 둠 10 → 13~16fps, 애프터버너
> 컴플리트 8fps. 60fps는 프레임당 5,197,920 사이클 안에 들어가야 하는데
> 24,000,000을 씁니다 — **4.6배**가 필요합니다.
>
> 코어는 런처에 남습니다(동작하고 정확합니다). 다만 **성능 과녁은 아닙니다.**
>
> **다시 파기 전에 반드시 읽을 것: `Core/Src/porting/md32x/CLAUDE.md`.**
> 측정으로 닫은 축 7개(쓰기경로·데이터로드 인라인·플래시 지연·다이나렉·ITCM·
> DTCM·QEMU 인구조사), 그중 둘은 만들어서 재보고 되돌린 것, 그리고 아직 안 쟀지만
> 재볼 값어치가 있는 것(68K·D32XR·2D 타이틀)이 거기 정리돼 있습니다.
>
> 아래 문서는 **QEMU 리그 기반 초기 최적화**의 이력입니다. 폐기된 게 아니라,
> 무엇을 어떻게 쟀는지의 기록으로 유효합니다.
>
> **기기 DWT 측정 이력(0726~0727)은 `docs/32X_DEVICE_MEASUREMENT_LOG.md`에 따로
> 있습니다** — 실패한 실험까지 전부, 가설·계측기·수치·판정 순으로. 특히 §0이
> "왜 GBA(M4A HLE) 같은 묘수가 여기엔 없었나"를 다룹니다.
...
**표: 최적화가 대상이 아닌 롬에서 해를 끼치지 않는지 확인.** `base/patched TOTAL` 은 QEMU
M7 리그가 센 호스트 명령 수이고(기기 사이클이 아니다), 프레임 수는 롬 이름 옆에 적혀 있다.
`fb 동일` 은 프레임버퍼 해시로, 같으면 화면이 비트 단위로 동일하다는 뜻이라 성능 수치를
비교할 자격이 생긴다. 다르면 두 빌드가 다른 그림을 그린 것이므로 Δ 는 의미가 없다.
`결정` 의 no-match 는 최적화가 이 롬에서 발동하지 않았다는 뜻이고, 그때 남는 Δ 는 프로브
자체의 오버헤드다.
