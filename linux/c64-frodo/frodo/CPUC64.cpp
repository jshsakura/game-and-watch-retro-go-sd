...
void MOS6510::illegal_op(uint8 op, uint16 at)
{
	...
	{static int n=0; if(n++<30) fprintf(stderr,"[ILLOP] op=%02x pc=%04x mem=%02x %02x %02x A=%02x X=%02x Y=%02x SP=%02x\n",op,at,read_byte(at),read_byte((uint16)(at+1)),read_byte((uint16)(at+2)),a,x,y,sp);}
...
void MOS6510::illegal_jump(uint16 at, uint16 to)
{
	...
	{static int n=0; if(n++<30) fprintf(stderr,"[ILLJMP] at=%04x to=%04x\n",at,to);}
	ShowRequester(illop_msg, "Reset");
	the_c64->Reset();
	Reset();
}
