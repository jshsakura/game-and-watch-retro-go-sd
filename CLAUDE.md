...
- **Faults mostly do NOT self-label — the BSOD gives you a title, a PC and an LR, and nothing else.** Outside the 32X core `SCB->SHCSR` is never written (grep it: no hit anywhere in `Core/` outside `main_md32x.c`), so BusFault/UsageFault/MemFault are all disabled and every one of them escalates to a plain **"Hardfault"**. The one exception, since the XN firewall (ad107058): while the 32X core is running, its entry code sets MEMFAULTENA and an execute-never MPU region over guest RAM, so an execution jump into guest RAM gets named "Memfault" with `CFSR.IACCVIOL` — and lands in the same breadcrumb trail as any other fault. Both the SHCSR bit and the region are cleared by the reset that follows every core exit, so other cores never see this. `CFSR/HFSR/BFAR/MMFAR/ABFSR` are not read and not printed. So the title tells you nothing about *why*, and "Hardfault" is compatible with an alignment fault, a null deref, and a wild store alike. Budget for that: the PC is the whole clue. (Super Metroid's unaligned `STRD` came up as a bare "Hardfault"; had UsageFault been enabled the title alone — "Usagefault" — would have named it.)
