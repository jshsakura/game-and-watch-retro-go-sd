...
- `CHEAT_CODES=1`, `COVERFLOW=1`, `SHARED_HIBERNATE_SAVESTATE=1`, `DISABLE_SPLASH_SCREEN=1`, `MSX_USE_BANK_2=1` — feature toggles. The Docker release build enables `COVERFLOW=1 SHARED_HIBERNATE_SAVESTATE=1 DISABLE_SPLASH_SCREEN=1 INTFLASH_BANK=2 CHEAT_CODES=1`.
  - **`FORCE_NOFRENDO` was listed here and does not exist.** It appears in no
    makefile, `NES_C_SOURCES` is empty, no nofrendo `.c` is in any source list,
    and `make help` does not mention it — NES is fceumm only. Make ignores an
    unknown variable silently, so passing it changed nothing and said nothing.
    `linux/Makefile.nes` still builds that dead core and no longer compiles:
    `nofrendo.h` declares `nofrendo_start`/`nes_insertcart` with the game-genie
    parameters and `nofrendo.c` defines them without, an inconsistency nothing
    in the firmware can catch because the firmware does not build the file.
...
- **An ISR above SysTick's priority must never enter a HAL wait loop.** `HAL_GetTick()` cannot advance while such a handler runs (SysTick is the lowest priority), so any "wait for ready / wait for disable" loop that times out on the tick is not slow there — it is infinite (guaranteed deadlock, found 2026-08-26: a priority-(0,0) TIM1 charger poll wedged a whole boot inside `ADC_Enable`). tests/test_adc_isr_wired.sh pins the current wiring; the rule is the general one — block in main context, never above the tick.
- **A probe that stops the CPU changes the thing it measures.** Halting the core lets interrupts go pending, and on resume the handler lands wherever the timing put it — the order of execution is reshuffled by the observation itself (2026-08-26: a 4-second breakpoint hold at the `wdog_enable` window manufactured a watchdog-unarmed boot that never happens naturally). Same family as "a warm SWD start is a detector": prefer no-halt reads, and treat any halt-adjacent anomaly as probe-made until proven otherwise.
...
- **Faults mostly do NOT self-label — the BSOD gives you a title, a PC and an LR, and nothing else.** Outside the 32X core `SCB->SHCSR` is never written (grep it: no hit anywhere in `Core/` outside `main_md32x.c`), so BusFault/UsageFault/MemFault are all disabled and every one of them escalates to a plain **"Hardfault"**. The one exception, since the XN firewall (ad107058): while the 32X core is running, its entry code sets MEMFAULTENA and an execute-never MPU region over guest RAM, so an execution jump into guest RAM gets named "Memfault" with `CFSR.IACCVIOL` — and lands in the same breadcrumb trail as any other fault. Both the SHCSR bit and the region are cleared by the reset that follows every core exit, so other cores never see this. The fault breadcrumbs (gw_crash_crumbs) read `CFSR`/`HFSR` (DR6/DR7) and, when `CFSR.BFARVALID`/`MMARVALID` say the hardware actually latched it, the faulting address into DR16 (`BFAR`) and DR17 (`MMFAR`) -- an invalid latch is written as `0xFFFFFFFF`, never a quiet 0 (the base4 six-boot crash loop died with `CFSR=0x8200`, BFARVALID set, and the address was lost because nothing read it). `ABFSR` is still not read. So the title tells you nothing about *why*, and "Hardfault" is compatible with an alignment fault, a null deref, and a wild store alike. Budget for that: the PC is the whole clue. (Super Metroid's unaligned `STRD` came up as a bare "Hardfault"; had UsageFault been enabled the title alone — "Usagefault" — would have named it.)
