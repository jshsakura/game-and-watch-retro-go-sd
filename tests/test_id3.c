...
/* The only seam music_id3.c reaches outside itself: a callback that keeps the
 * audio DMA fed while a long tag walk holds the SD. It is playback plumbing,
 * not parsing, so the parser under test is still the real file. */
...
static void test_cover(void)
{
    ...
    /* The reader locates the cover instead of loading it: the image never
     * lands in RAM, the caller stream-decodes it from (off, len). So this
     * checks the offset actually points AT the image bytes in the file --
     * a length alone would pass with the offset off by the frame header. */
    long off = -1, len = -1;
    bool png_b = true;
    int found = id3_locate_cover("/tmp/t_id3_cover.mp3", &off, &len, &png_b);
    CHECK(found == 1, "cover located");
    CHECK(len == 12, "cover byte length == jpeg length");
    CHECK(!png_b, "cover detected as JPEG (not png)");

    unsigned char at[2] = { 0, 0 };
    if (found && off >= 0) {
        FILE *cf = fopen("/tmp/t_id3_cover.mp3", "rb");
        if (cf) {
            if (fseek(cf, off, SEEK_SET) == 0)
                (void)!fread(at, 1, sizeof(at), cf);
            fclose(cf);
        }
    }
    CHECK(at[0] == 0xFF && at[1] == 0xD8, "offset points at the JPEG SOI");
}
...
static void test_no_tag(void)
{
    ...
    long off, len;
    bool png_b;
    CHECK(id3_locate_cover("/tmp/t_id3_none.mp3", &off, &len, &png_b) == 0, "no ID3 -> no cover");
}
