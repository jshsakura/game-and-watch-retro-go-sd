...
CORE_SRCS="external/sm/src/snes/apu.c external/sm/src/snes/cart.c \
  external/sm/src/snes/cx4_hle.c external/sm/src/snes/dma.c external/sm/src/snes/dsp.c \
  external/sm/src/snes/input.c external/sm/src/snes/ppu.c \
  external/sm/src/snes/snes.c external/sm/src/snes/snes_other.c \
  external/sm/src/snes/spc.c external/sm/src/tracing.c external/sm/src/snes/rc_dispatch.c"
...
gcc -o "$O/dump" "$O"/{apu,cart,cx4_hle,dma,dsp,input,ppu,snes,snes_other,spc,tracing,rc_dispatch}.o \
    "$O/cpu_hist.o" "$O/harness_hist.o" -lm
SNES_HISTDUMP="$O/hist.bin" SNES_SITEDUMP="$O/sites.bin" SNES_CARTDUMP="$O/cart.bin" \
...
gcc -o "$O/baseline" "$O"/{apu,cart,cx4_hle,dma,dsp,input,ppu,snes,snes_other,spc,tracing,rc_dispatch}.o \
...
gcc -o "$O/hybrid" "$O"/{apu,cart,cx4_hle,dma,dsp,input,ppu,snes,snes_other,spc,tracing,rc_dispatch}.o \
