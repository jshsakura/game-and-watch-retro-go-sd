...
/* ---- APU-wait idle detection (fork: idle-skip fps-ceiling probe) ----------
 * g_apu_read is set by the sed-instrumented snes_readBBus whenever the 65816
 * reads an APU out-port ($2140-$2143). snes_idle_op() is called once per 65816
 * opcode from the sed-instrumented cpu.c hook, with the opcode's 24-bit address.
 * An opcode is "APU-wait" if it is a member of a tight loop (<=LOOPMAX opcodes)
 * in which an APU-port read occurred -- exactly the pattern an idle-skip detects
 * and fast-forwards past. We also count the bare read opcodes as a hard floor. */
int g_apu_read;
#define LOOPMAX 16
static uint32_t idl_pc[LOOPMAX];
static uint8_t  idl_rd[LOOPMAX];
static int      idl_head, idl_prim;
uint64_t g_ops_total, g_ops_wait, g_ops_read;

void snes_idle_op(uint32_t pc) {
  int prev = (idl_head - 1 + LOOPMAX) % LOOPMAX;
  if (idl_prim > 0 && g_apu_read) { idl_rd[prev] = 1; g_ops_read++; }
  g_apu_read = 0;
  ...
  int found_len = 0;
  for (int d = 1; d < LOOPMAX; d++) {
    int j = (idl_head - d + LOOPMAX) % LOOPMAX;
    if (idl_pc[j] == pc) { found_len = d; break; }
  }
  if (found_len && idl_prim >= LOOPMAX) {
    int hit = 0;
    for (int d = 0; d < found_len; d++) {
      int j = (idl_head - d + LOOPMAX) % LOOPMAX;
      if (idl_rd[j]) { hit = 1; break; }
    }
    if (hit) g_ops_wait++;
  }
  idl_head = (idl_head + 1) % LOOPMAX;
  if (idl_prim < LOOPMAX) idl_prim++;
}
...
int main(int argc, char **argv) {
  ...
  uint64_t pf_ops=0, pf_wait=0, pf_read=0, gp_ops=0, gp_wait=0, gp_read=0;
  ...
  for (int i = 0; i < frames; i++) {
    ...
    { uint64_t dops=g_ops_total-pf_ops, dwait=g_ops_wait-pf_wait, dread=g_ops_read-pf_read;
      pf_ops=g_ops_total; pf_wait=g_ops_wait; pf_read=g_ops_read;
      if (i >= WARMUP) { gp_ops+=dops; gp_wait+=dwait; gp_read+=dread; gp_frames++; }
      if ((i+1)%100==0) {
        int L=0; for(int q=0;q<320*240;q++) if(g_fb[q]) L++;
        fprintf(stderr,"w%5d ops/f=%llu wait=%.1f%% read=%.2f%% lit=%d\n",
          i+1,(unsigned long long)dops, dops?100.0*dwait/dops:0.0, dops?100.0*dread/dops:0.0, L);
      }
    }
  }
  ...
  printf("[idle-probe] gameplay frames=%d  ops/frame=%llu  APU-wait=%.2f%%  read-floor=%.3f%%\n",
         gp_frames, gp_frames?(unsigned long long)(gp_ops/gp_frames):0ULL,
         gp_ops?100.0*gp_wait/gp_ops:0.0, gp_ops?100.0*gp_read/gp_ops:0.0);
#ifdef SNES_PC_HISTOGRAM
  dump_pchist();
#endif
  return 0;
