...
$CC -c $ARCH $OPT $DEF $INC -iquote $SM/src/snes -w "$NGEN/apu_wire.c" -o "$OUT/apu_wire.o"
# spc_player_gen.c: zero-copy, dialect-parameterized player (device's own copy).
$CC -c $ARCH $OPT $DEF $INC $SMINC -include "$HLE/nspc_config.h" -w "$NGEN/spc_player_gen.c" -o "$OUT/spc_player_gen.o"
$CC -c $ARCH $OPT $DEF $INC $SMINC -I"$HLE" -w "$HLE/nspc_variant.c" -o "$OUT/nspc_variant.o"
# THE FILE UNDER TEST: the firmware's own tools/nspc_audio_wire/nspc_wire.c,
# never edited by this script.
$CC -c $ARCH $OPT $DEF $INC $SMINC $NSPCINC -w "$HERE/nspc_wire.c" -o "$OUT/nspc_wire.o"
$CC -c $ARCH $OPT $DEF $INC -w "$HERE/rig_snes_device_wire.c" -o "$OUT/rig_snes_device_wire.o"
