...
/* WAI dot accounting: a dot spent with the CPU in WAI (waiting for an
 * interrupt) is a dot the fast-forward lever could skip entirely. Count
 * dots-in-WAI vs total dots so the ratio is per-ROM comparable. */
static unsigned long long g_wai_dots = 0, g_tot_dots = 0;
...
static void run_dots(Snes *snes, int dots) {
  while (dots > 0) {
    ...
    g_tot_dots += step;
    if (snes->cpu->waiting) g_wai_dots += step;
...
int main(int argc, char **argv) {
  ...
  {
    double wai_pct = g_tot_dots ? (100.0 * (double)g_wai_dots / (double)g_tot_dots) : 0.0;
    printf("%s\tOK\twai=%.1f\tdots=%llu\t%s\n", base, wai_pct,
           (unsigned long long)g_tot_dots, mlen ? matches : "-");
  }
  return 0;
}
