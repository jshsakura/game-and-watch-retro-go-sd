#!/bin/bash
# build_cache.sh — build the SegaCD host boot harness with no-cache instruction counter.
...
#
# Output: /tmp/boot_nocache  (run via bench.sh with SCD_* env toggles)
set -uo pipefail
...
gcc -O2 -DLINUX_EMU -DTARGET_GNW -DIS_LITTLE_ENDIAN \
  ...
  -lm -o /tmp/boot_nocache 2>/tmp/build_cache.err
status=$?
if [ $status -ne 0 ]; then
...
echo "BUILD_OK -> /tmp/boot_nocache"
