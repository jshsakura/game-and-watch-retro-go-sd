...
#
# Output: /tmp/boot_verify  (run via bench.sh with SCD_* env toggles)
set -uo pipefail
...
gcc -O2 -DLINUX_EMU -DTARGET_GNW -DIS_LITTLE_ENDIAN \
  ...
  -DSCD_BENCH_VDP \
  -DHOOK_CPU -DSCD_CACHE -DSCD_CACHE_ONLY -DSCD_CACHE_VERIFY \
  ...
  -lm -o /tmp/boot_verify 2>/tmp/build_cache.err
status=$?
if [ $status -ne 0 ]; then
