...
#define SUB_CYCLES_PER_FRAME 208333u /* sub-68K 12.5 MHz / 60 fps */
...
int main(void)
{
    ...
    for (int frame = 0; frame < RIG_FRAMES; frame++) {
        ...
        /* --- Sega CD dual-68K cost ---
         * The sub-68K runs its own 12.5 MHz budget/frame. Measure that cost as
         * the marginal cost of interpreting SUB_CYCLES_PER_FRAME more 68K cycles
         * over the real game's instruction mix — snapshot/restore so the main
         * machine state is untouched (crash-free, no shared-RAM corruption).
         * This is the "sub fully busy" UPPER bound; idle-skip drives it toward 0
         * whenever the sub is spin-waiting, so real gameplay sits below this. */
        {
            static m68ki_cpu_core save;
            memcpy(&save, &m68k, sizeof(m68ki_cpu_core));
            unsigned int base = m68k.cycles;
            uint32_t s0 = rig_timer_now();
            m68k_run(base + SUB_CYCLES_PER_FRAME);
            uint32_t s1 = rig_timer_now();
            memcpy(&m68k, &save, sizeof(m68ki_cpu_core));   /* main untouched */
            win_sub += (uint32_t)(s1 - s0);
        }
        ...
        if ((frame + 1) % RIG_WINDOW == 0) {
            ...
            uint64_t sub_i  = win_sub * ipt_x1000 / 1000 / RIG_WINDOW;
            uint64_t dual_i = emu_i + sub_i;
            unsigned mul100  = (unsigned)(dual_i * 100 / (emu_i ? emu_i : 1));
            printf("w%05d emu=%lu sub=%lu dual=%lu (x%u.%02u) fb=%08x\n",
                   frame + 1, (unsigned long)emu_i, (unsigned long)sub_i,
                   (unsigned long)dual_i, mul100 / 100, mul100 % 100, (unsigned)h);
            tot_emu += win_emu; tot_sub += win_sub;
            win_emu = win_blit = win_sub = 0;
        }
    }

    uint64_t frames = (RIG_FRAMES / RIG_WINDOW) * RIG_WINDOW;
    if (!frames) frames = 1;
    uint64_t ae = tot_emu * ipt_x1000 / 1000 / frames;
    uint64_t as = tot_sub * ipt_x1000 / 1000 / frames;
    unsigned amul = (unsigned)((ae + as) * 100 / (ae ? ae : 1));
    printf("[mcd-qemu] done %d frames RUNHASH=%08x avg emu=%lu sub=%lu dual=%lu (x%u.%02u)\n",
           RIG_FRAMES, (unsigned)run_hash, (unsigned long)ae, (unsigned long)as,
           (unsigned long)(ae + as), amul / 100, amul % 100);
    return 0;
}
