/* HLE-measurement variant of rig_snes.c: identical machine, but apu.c is compiled
 * as apu_hle.c (sed-instrumented) so the executed-instruction time spent inside
 * spc_runOpcode accumulates into g_spc_ticks. This isolates the SPC700 CPU
 * interpreter cost -- exactly what N-SPC HLE removes (spc_player replaces the
 * SPC700 CPU; the S-DSP mixer stays). A third per-window figure `spc700` is
 * printed next to emu/apu on the same insn/frame scale.
 *
 * The two rig_timer_now() calls per SPC opcode add a little overhead to the emu
 * window, so the HEADLINE baseline emu/apu should be taken from the stock rig;
 * this rig's job is the spc700 fraction. (main prints both so they can be
 * cross-checked -- instrumented emu+apu should be within a few % of stock.)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* accumulated by the sed-instrumented apu_hle.c */
uint64_t g_spc_ticks = 0;
...
/* ---- event loop, verbatim from rig_snes.c / snes_main.c ---- */
...
int main(void) {
  ...
  *(volatile uint32_t *)0xE000ED88 |= (0xFu << 20);
  ...
  uint64_t prev_spc = 0, tot_spc = 0;
  /* gameplay-only accumulators: windows with lit>0 */
  uint64_t gp_emu = 0, gp_apu = 0, gp_spc = 0; int gp_windows = 0;
  ...
  for (int frame = 0; frame < RIG_FRAMES; frame++) {
    ...
    if ((frame + 1) % RIG_WINDOW == 0) {
      uint64_t win_spc = g_spc_ticks - prev_spc; prev_spc = g_spc_ticks;
      uint64_t emu_i = win_emu * ipt_x1000 / 1000 / RIG_WINDOW;
      uint64_t apu_i = win_apu * ipt_x1000 / 1000 / RIG_WINDOW;
      uint64_t spc_i = win_spc * ipt_x1000 / 1000 / RIG_WINDOW;
      int lit = 0;
      for (int q = 0; q < 320 * 240; q++) if (g_fb[q]) lit++;
      printf("w%05d emu=%lu apu=%lu spc700=%lu insn/frame fb=%08lx lit=%d\n",
             frame + 1, (unsigned long)emu_i, (unsigned long)apu_i,
             (unsigned long)spc_i, (unsigned long)(uint32_t)h, lit);
      tot_emu += win_emu; tot_apu += win_apu; tot_spc += win_spc;
      if (lit > 0) { gp_emu += win_emu; gp_apu += win_apu; gp_spc += win_spc; gp_windows++; }
      win_emu = win_apu = 0;
    }
  }
  ...
  uint64_t a_emu = tot_emu * ipt_x1000 / 1000 / frames;
  uint64_t a_apu = tot_apu * ipt_x1000 / 1000 / frames;
  uint64_t a_spc = tot_spc * ipt_x1000 / 1000 / frames;
  printf("[snes-qemu-hle] done %d frames STATEHASH=%08lx avg emu=%lu apu=%lu spc700=%lu insn/frame\n",
         RIG_FRAMES, (unsigned long)(uint32_t)(run_hash ^ sh),
         (unsigned long)a_emu, (unsigned long)a_apu, (unsigned long)a_spc);

  /* gameplay-only (lit>0) headline */
  uint64_t gframes = (uint64_t)gp_windows * RIG_WINDOW;
  if (gp_windows == 0) { printf("[hle] NO gameplay windows (lit>0) seen\n"); return 0; }
  uint64_t g_emu = gp_emu * ipt_x1000 / 1000 / gframes;
  uint64_t g_apu = gp_apu * ipt_x1000 / 1000 / gframes;
  uint64_t g_spc = gp_spc * ipt_x1000 / 1000 / gframes;
  uint64_t base = g_emu + g_apu;
  uint64_t hle  = (base > g_spc) ? base - g_spc : 0;
  /* fps*100 = (340MHz / insn_per_frame); 340e6*100 = 34,000,000,000 */
  unsigned long base_fps100 = (unsigned long)(34000000000ull / (base ? base : 1));
  unsigned long hle_fps100  = (unsigned long)(34000000000ull / (hle ? hle : 1));
  unsigned long spd100 = base ? (unsigned long)((uint64_t)base * 100 / (hle ? hle : 1)) : 0;
  printf("[hle] GAMEPLAY(%d win, lit>0): emu=%lu apu=%lu spc700=%lu insn/frame\n",
         gp_windows, (unsigned long)g_emu, (unsigned long)g_apu, (unsigned long)g_spc);
  printf("[hle] baseline_total=%lu insn/frame -> %lu.%02lu fps @340MHz\n",
         (unsigned long)base, base_fps100 / 100, base_fps100 % 100);
  printf("[hle] hle_total=%lu insn/frame (spc700 removed) -> %lu.%02lu fps @340MHz  (x%lu.%02lu, insn!=cycle, no cache model)\n",
         (unsigned long)hle, hle_fps100 / 100, hle_fps100 % 100, spd100 / 100, spd100 % 100);
  return 0;
}
