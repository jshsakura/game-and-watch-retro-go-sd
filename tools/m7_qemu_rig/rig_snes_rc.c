...
int main(void) {
  ...
  *(volatile uint32_t *)0xE000ED88 |= (0xFu << 20);
  ...
  /* Host builds: snes_loadRom malloc'd a second, pow2-expanded copy. The ELF
   * already carries the blob — point cart->rom back at it and free the copy.
   * GNW_SNES_CORE builds: cart_load points at `rom` IN PLACE (zero-copy,
   * cart_fold() mirrors at access time), so this free would release the
   * linked-in blob — a non-heap pointer — and the corrupted allocator then
   * spins forever inside init (77% CPU, nothing printed past the boot cal). */
  free(snes->cart->rom);
  snes->cart->rom = rom;
#endif
