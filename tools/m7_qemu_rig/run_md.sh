...
#
#   bash tools/m7_qemu_rig/run_md.sh <rom.bin|md|gen> [frames]
#
...
ROM="${1:?usage: run_md.sh <rom.bin> [frames]}"
FRAMES="${2:-3000}"
...
SRCS="$GW/cpus/M68K/m68kcpu.c $GW/cpus/Z80/Z80.c $GW/sound/z80inst.c \
      ...
      $RIG/rig_runtime.c $RIG/rig_md.c"
...
$CC $ARCH -T "$RIG/mps2_an500.ld" -nostartfiles -Wl,--gc-sections \
    ...
    $OBJS -lm -o "$OUT/rig_md.elf"

arm-none-eabi-size "$OUT/rig_md.elf"

timeout 1800 qemu-system-arm -machine mps2-an500 -nographic -semihosting \
    -icount shift=0,align=off,sleep=off -kernel "$OUT/rig_md.elf"
