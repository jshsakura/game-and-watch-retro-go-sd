#!/bin/bash
# Spin-skip (NMI-wait exact replay) on the hard-float M7 rig, stock interpreter.
#   bash tools/m7_qemu_rig/run_snes_spin.sh <rom.smc> [frames]
# Gate: STATEHASH must equal the stock run_snes_hf.sh for the same ROM/frames.
set -euo pipefail
cd "$(dirname "$0")/../.."

ROM="${1:?usage: run_snes_spin.sh <rom.smc> [frames]}"
FRAMES="${2:-1200}"
...
arm-none-eabi-size "$OUT/rig_snes_spin.elf"

timeout 1800 qemu-system-arm -machine mps2-an500 -nographic -semihosting \
    -icount shift=0,align=off,sleep=off -kernel "$OUT/rig_snes_spin.elf"
