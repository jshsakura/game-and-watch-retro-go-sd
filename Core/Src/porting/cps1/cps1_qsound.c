...
#include "cps1_qsound.h"

#define CLAMP(x, low, high)  (((x) > (high)) ? (high) : (((x) < (low)) ? (low) : (x)))

// ============================================================================

static const int16_t cps1_qsound_dry_mix_table[33] = {
...
static const int16_t cps1_qsound_wet_mix_table[33] = {
...
static const int16_t cps1_qsound_linear_mix_table[33] = {
...
static const int16_t cps1_qsound_filter_data[5][95] = {
	{	// d53 - 0
...
static const int16_t cps1_qsound_filter_data2[209] = {
...
static const int16_t cps1_qsound_adpcm_step_table[16] = {
...
static void init_pan_tables(struct cps1_qsound_chip *chip);
static void init_register_map(struct cps1_qsound_chip *chip);

static void state_init(struct cps1_qsound_chip *chip);
static void state_refresh_filter_1(struct cps1_qsound_chip *chip);
static void state_refresh_filter_2(struct cps1_qsound_chip *chip);
static void state_normal_update(struct cps1_qsound_chip *chip);

static inline int16_t get_sample(struct cps1_qsound_chip *chip, uint16_t bank,uint16_t address);
static inline int16_t* get_filter_table(struct cps1_qsound_chip *chip, uint16_t offset);
static inline int16_t pcm_update(struct cps1_qsound_chip *chip, int voice_no, int32_t *echo_out);
static inline void adpcm_update(struct cps1_qsound_chip *chip, int voice_no, int nibble);
static inline int16_t echo(struct cps1_qsound_echo *r,int32_t input);
static inline int32_t fir(struct cps1_qsound_fir *f, int16_t input);
static inline int32_t delay(struct cps1_qsound_delay *d, int32_t input);
static inline void delay_update(struct cps1_qsound_delay *d);
...
void cps1_qsound_w(struct cps1_qsound_chip *chip, uint8_t offset, uint8_t data)
{
	switch (offset)
	{
		...
		case 1:
			...
			break;
		case 2:
			cps1_qsound_write_data(chip, data, chip->data_latch);
			break;
		default:
			break;
	}
...
static void init_pan_tables(struct cps1_qsound_chip *chip)
{
	...
	{
		// dry mixing levels
		chip->pan_tables[PANTBL_LEFT][PANTBL_DRY][i] = cps1_qsound_dry_mix_table[i];
		chip->pan_tables[PANTBL_RIGHT][PANTBL_DRY][i] = cps1_qsound_dry_mix_table[32-i];
		// wet mixing levels
		chip->pan_tables[PANTBL_LEFT][PANTBL_WET][i] = cps1_qsound_wet_mix_table[i];
		chip->pan_tables[PANTBL_RIGHT][PANTBL_WET][i] = cps1_qsound_wet_mix_table[32-i];
		// linear panning, only for dry component. wet component is muted.
		chip->pan_tables[PANTBL_LEFT][PANTBL_DRY][i+0x30] = cps1_qsound_linear_mix_table[i];
		chip->pan_tables[PANTBL_RIGHT][PANTBL_DRY][i+0x30] = cps1_qsound_linear_mix_table[32-i];
	}
}
...
static inline int16_t get_sample(struct cps1_qsound_chip *chip, uint16_t bank,uint16_t address)
{
	...
	/* GNW: dispatch across the four cached-flash chips. */
	{
		uint32_t chip_i = rom_addr >> CPS1_QSOUND_ROM_CHIP_SHIFT;
		if (chip_i >= CPS1_QSOUND_ROM_CHIPS || chip->rom_chip[chip_i] == NULL)
			return 0;
		sample_data = chip->rom_chip[chip_i][rom_addr & ((1u << CPS1_QSOUND_ROM_CHIP_SHIFT) - 1u)];
	}
...
static inline int16_t* get_filter_table(struct cps1_qsound_chip *chip, uint16_t offset)
{
	...
	if (offset >= 0xf2e && offset < 0xfff)
		return (int16_t*)&cps1_qsound_filter_data2[offset-0xf2e];	// overlapping filter data

	index = (offset-0xd53)/95;
	if(index >= 0 && index < 5)
		return (int16_t*)&cps1_qsound_filter_data[index];	// normal tables
...
static inline int16_t pcm_update(struct cps1_qsound_chip *chip, int voice_no, int32_t *echo_out)
{
	struct cps1_qsound_voice* v = &chip->voice[voice_no];
...
static inline void adpcm_update(struct cps1_qsound_chip *chip, int voice_no, int nibble)
{
	struct cps1_qsound_adpcm *v = &chip->adpcm[voice_no];
	...
	v->step_size = (cps1_qsound_adpcm_step_table[8+step] * v->step_size) >> 6;
