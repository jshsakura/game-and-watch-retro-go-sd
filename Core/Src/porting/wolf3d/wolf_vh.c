...
/* G&W port: no LTDC hardware CLUT. Hand the freshly-drawn 8-bit frame and the
 * RGB888 palette LUT to the firmware-side presenter, which does the software
 * 8bit->RGB565 expansion into the LCD buffer, input poll and pause menu. */
extern void gw_wolf_present(const uint8_t *fb320x200, const uint32_t *pal888);

void VH_UpdateScreen()
{
    // keep back->front copy so any engine code reading `screen` sees the frame
