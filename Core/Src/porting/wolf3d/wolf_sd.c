...
/* G&W port: the original board fed audio via a DMA1_Stream1 -> DAC double
 * buffer ISR. The G&W has no such DAC path; audio is stubbed for now, so this
 * handler is neutered and renamed (removed from the vector table) to drop the
 * DMA1/SCB hardware references. (void) keeps the globals referenced-safe. */
static void wolf_audio_dma_isr_unused(void)
{
    (void)snd_playing; (void)snd_size; (void)snd_scale; (void)sndpagebufpos;
}
