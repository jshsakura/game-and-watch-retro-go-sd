...
/* SrcSize is the number of JPEG bytes actually present at SrcAddress. The
 * hardware decoder reads its input straight out of that memory, so a size that
 * overstates the buffer lets a truncated or corrupt image walk off the end of
 * it. Every decode below returns 0 on success and non-zero when the image was
 * rejected — too large for the output buffer, malformed, or timed out. Callers
 * must treat non-zero as "no image" instead of trusting the output buffer. */
