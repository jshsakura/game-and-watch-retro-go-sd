...
static unsigned int sub_prg_read16(unsigned int address);
static void sub_prg_write16(unsigned int address, unsigned int data);
...
void segacd_sub_build_memory_map(void)
{
    ...
    /* $000000-$07FFFF : PRG-RAM, 8 x 64 KiB guest pages each with its OWN host
     * backing pointer (gate-6: pages 0..6 static AXI overlay BSS, page 7 DTCM
     * heap — non-contiguous by design). Direct .base access, NULL handlers =
     * raw memory under gwenesis's byte-swapped convention. The old split
     * (first 128 KiB direct / rest via SD file-paging handlers) is gone. */
    for (int p = 0; p < SEGACD_PRG_PAGE_COUNT; p++) {
        map[p].base = SCD.prg_page[p];
...
/* ---- PRG-RAM access through the page[8] table (gate-6) ----
 *
 * The old handlers implemented $020000-$07FFFF as one-byte fopen/fseek/fread
 * cycles on /tmp/scd_prg.bin — the invalid 128 KiB compromise the
 * reassessment closed. All 512 KiB are resident now but NON-CONTIGUOUS
 * (7 pages AXI overlay BSS + 1 page DTCM heap), so every byte access goes
 * through the page table: no code path may ever hold a fake contiguous
 * base pointer, or the old bug comes back disguised.
 *
 * `^1` keeps gwenesis's byte-swapped storage convention (LSB_FIRST host,
 * big-endian bus): byte n of a big-endian word pair lives at n^1. It is a
 * bit-0 XOR, so it cannot cross a 64 KiB page boundary, and word accesses
 * are aligned pairs — page-local by construction. */
static inline uint8_t *scd_prg_ptr(unsigned int address)
{
    return SCD.prg_page[(address >> 16) & (SEGACD_PRG_PAGE_COUNT - 1)]
                       + ((address & 0xFFFF) ^ 1);
}

unsigned int sub_prg_read8(unsigned int address) {
    return *scd_prg_ptr(address);
}
static unsigned int sub_prg_read16(unsigned int address) {
    return (sub_prg_read8(address) << 8) | sub_prg_read8(address + 1);
}
void sub_prg_write8(unsigned int address, unsigned int data) {
    *scd_prg_ptr(address) = (uint8_t)data;
}
static void sub_prg_write16(unsigned int address, unsigned int data) {
    sub_prg_write8(address, data >> 8); sub_prg_write8(address + 1, data & 0xFF);
}
