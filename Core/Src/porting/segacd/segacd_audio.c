...
extern void *ahb_only_malloc(unsigned size); /* gw_malloc.c — strict AHB pool */

void segacd_audio_mix(int16_t *dst, const int16_t *ym, const int16_t *sn,
                      int frames, int volume)
{
    /* Gate-6 placement contract (docs/SEGACD_REASSESSMENT_2026-09-14.md): the
     * CD-DA mix buffer is the 4,800 B that moved from overlay BSS to AHB to
     * close the AXI -1,244 shortfall; pcm[] stays in the AXI overlay BSS the
     * probe accounted for. Allocated lazily on first submit — the AHB pool is
     * rebased (ahb_set_core_base) long before the first frame, so this lands
     * at the probe-verified 0x3001b800..0x3001cac0 range under the audio
     * ceiling. A NULL (pool exhausted) degrades to CD-DA-off for this submit
     * rather than mixing through a wild pointer. */
    static int16_t *cdda;
    static int16_t pcm[SEGACD_AUDIO_MAX * 2];

    if (frames > SEGACD_AUDIO_MAX) frames = SEGACD_AUDIO_MAX;

    if (!cdda) cdda = ahb_only_malloc(SEGACD_AUDIO_MAX * 2 * sizeof(int16_t));
    int cn = cdda ? segacd_cdda_fill(cdda, frames) : 0; /* CD-DA from SD */
