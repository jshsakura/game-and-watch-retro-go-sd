...
/* device allocators (ahb_malloc = AHB SRAM preferring AXI, ram_malloc = AXI
 * RAM_EMU pool, ahb_only_malloc = AHB SRAM strictly, fails rather than fall
 * through to AXI) */
extern void *ram_malloc(size_t);
extern void *ahb_malloc(size_t);
extern void *ahb_only_malloc(size_t);
 ...
 * in segacd_init(). This is the exact non-contiguous layout the
 * SEGACD_RAM_PROBE exercised on device (G4/G5 pass, 2026-09-16: prg0..6 at
 * 0x240ef23c..0x2408f23c descending, page 7 in DTCM). Plain .bss — the
 * .overlay_segacd_bss wildcard captures build/segacd/*.o, so no section
 * attributes needed. */
static uint8_t scd_prg_page_axi[SEGACD_PRG_PAGE_COUNT - 1][SEGACD_PRG_PAGE_SIZE];
static uint8_t scd_word_ram[SEGACD_WORD_RAM_SIZE];
...
void segacd_init(void)
{
     ...
     * overlay BSS in AXI's single-FB span; PRG page 7 comes from the DTCM
     * heap (the 24,696 B margin contract — assert-style check belongs to the
     * caller's bring-up breadcrumbs if this fails); PCM goes to the
     * core-rebased AHB pool via ahb_only_malloc (base 0x30000000, rebased by
     * app_main_segacd before power_on() so the YM2612 tables claim the same
     * pool first). */
    {
        int p;
        for (p = 0; p < SEGACD_PRG_PAGE_COUNT - 1; p++)
            SCD.prg_page[p] = scd_prg_page_axi[p];
        SCD.prg_page[SEGACD_PRG_PAGE_COUNT - 1] = malloc(SEGACD_PRG_PAGE_SIZE);
        SCD.word_ram = scd_word_ram;
        SCD.pcm_ram  = ahb_only_malloc(SEGACD_PCM_RAM_SIZE);
    }
...
{
    {
        ...
        for (p = 0; p < SEGACD_PRG_PAGE_COUNT; p++)
            if (SCD.prg_page[p]) memset(SCD.prg_page[p], 0, SEGACD_PRG_PAGE_SIZE);
    }
