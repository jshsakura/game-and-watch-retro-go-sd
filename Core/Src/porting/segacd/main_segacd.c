...
int app_main_segacd(uint8_t load_state, uint8_t start_paused, int8_t save_slot)
{
    ...
    /* segacd_hold_screen() waits for a keypress and RETURNS -- and returning
     * from app_main is not a way out: emulator_start() has already torn the
     * launcher down (ahb_init/itc_init, ram_start=0, emulators and systems
     * NULLed) before handing over, so the unwind lands in state that no longer
     * exists and HardFaults. Every failure path below therefore ends in
     * odroid_system_switch_app(0), the way every other core exits. Keeping the
     * diagnostic screen and dropping that reboot would have traded a silent
     * failure for a crash right after the message. */
