...
/* PRG RAM is page[8]-backed, NOT one contiguous host block (gate-6 contract,
 * docs/SEGACD_REASSESSMENT_2026-09-14.md): gwenesis maps the sub-68K address
 * space in 64 KiB pages, so each guest page gets its own host pointer. Pages
 * 0..6 are static overlay BSS in AXI; page 7 is claimed from the DTCM heap at
 * init. SEGACD_PRG_RAM_SIZE stays the full 512 KiB guest semantics — every
 * access must go through SCD.prg_page[(addr >> 16) & 7]; a fake contiguous
 * base pointer would recreate the 128 KiB file-paging bug this port deleted. */
#define SEGACD_PRG_PAGE_SIZE  (64  * 1024)   /* one guest page = one host backing */
#define SEGACD_PRG_PAGE_COUNT 8              /* $020000-$07FFFF */
#define SEGACD_PRG_RAM_SIZE   (SEGACD_PRG_PAGE_SIZE * SEGACD_PRG_PAGE_COUNT)
...
typedef struct {
    ...
    uint8_t  *prg_page[SEGACD_PRG_PAGE_COUNT];  /* 7x AXI overlay BSS + 1x DTCM heap */
...
unsigned int sub_prg_read8(unsigned int address);   /* page[8] table, ^1 swap */
