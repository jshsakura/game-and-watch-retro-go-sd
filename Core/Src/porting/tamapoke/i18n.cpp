...
static const char *const STRINGS[LANG_COUNT][STR_COUNT] = {
  // ---------------- ES ----------------
  {
    ...
    "HUEVO", "Huevo legendario!?", "Huevo raro!", "Pulsa A en el huevo...", "Se mueve!", "Esta a punto!",
    "POKEDEX %u/151",
    "%s%s Nv.%u",
    "Soltar a %s?", "SI", "NO",
    ...
    "%s   EDAD %lud", "A en el nombre: renombrar",
    "COMBATE", "FUE", "DEF", "VEL", "PES", "ENTRENAR FUERZA",
    "MEDALLAS %d/%d", "B: volver",
    "NOMBRE:", "B para volver",
    "COM", "FEL", "ENE", "LIM",
    "REC %u",
  ...
  // ---------------- EN ----------------
  {
    ...
    "EGG", "Legendary egg!?", "Rare egg!", "Press A on the egg...", "It moves!", "Almost there!",
    "POKEDEX %u/151",
    "%s%s Lv.%u",
    "Release %s?", "YES", "NO",
    ...
    "%s   AGE %lud", "A on the name: rename",
  ...
  // ---------------- FR ----------------
  {
    ...
    "OEUF", "Oeuf legendaire!?", "Oeuf rare!", "Appuie sur A...", "Il bouge!", "Presque la!",
    "POKEDEX %u/151",
    "%s%s Niv.%u",
    "Relacher %s?", "OUI", "NON",
    ...
    "%s   AGE %lud", "A sur le nom: renommer",
    "COMBAT", "ATQ", "DEF", "VIT", "PDS", "ENTRAINER FORCE",
    "MEDAILLES %d/%d", "B: retour",
    "NOM:", "B pour revenir",
    "NOUR", "JOIE", "ENE", "HYG",
    "REC %u",
  ...
  // ---------------- DE ----------------
  {
    ...
    "%s   ALTER %lud", "A auf Name: umbenennen",
    "KAMPF", "ANG", "VER", "INI", "GEW", "KRAFT TRAINIEREN",
    "MEDAILLEN %d/%d", "B: zurueck",
    "NAME:", "B fuer zurueck",
    "ESS", "FRO", "ENE", "HYG",
    "REK %u",
  ...
  // ---------------- IT ----------------
  {
    ...
    "UOVO", "Uovo leggendario!?", "Uovo raro!", "Premi A sull'uovo...", "Si muove!", "Ci siamo quasi!",
    "POKEDEX %u/151",
    "%s%s Lv.%u",
    "Liberare %s?", "SI", "NO",
    ...
    "%s   ETA %lud", "A sul nome: rinomina",
    "LOTTA", "ATT", "DIF", "VEL", "PES", "ALLENA FORZA",
    "MEDAGLIE %d/%d", "B: indietro",
    "NOME:", "B per tornare",
    "CIB", "GIO", "ENE", "IGI",
    "REC %u",
  ...
  // ---------------- PT ----------------
  {
    ...
    "OVO", "Ovo lendario!?", "Ovo raro!", "Aperte A no ovo...", "Mexe-se!", "Quase la!",
    "POKEDEX %u/151",
    "%s%s Niv.%u",
    "Soltar %s?", "SIM", "NAO",
    ...
    "%s   IDADE %lud", "A no nome: renomear",
    "COMBATE", "ATQ", "DEF", "VEL", "PES", "TREINAR FORCA",
    "MEDALHAS %d/%d", "B: voltar",
    "NOME:", "B para voltar",
    "COM", "ALE", "ENE", "HIG",
    "REC %u",
  ...
  // specifiers match the English row position for position, which a
  // reordering would silently break at printf time. Plain-form throughout,
  // because the pet talks to you rather than at you.
  {
    "진화하고 있어!", "냠냠!", "마음에 들어해!", "배고파해...", "씻고 싶어해!", "기운이 없어...", "시무룩해...",
    "살이 좀 쪘어...", "이로치다!!", "기분이 좋아", "고마웠어! 잘 가", "도망가 버렸어...", "안녕! 손을 흔들고 있어...",
    "알", "전설의 알!?", "희귀한 알!", "알을 눌러봐...", "움직인다!", "곧 나올 것 같아!", "도감 %u/151",
    "%s%s Lv.%u", "%s 놓아줄까?", "응", "아니", "%u타", "힘 +%u", "신기록!", "최고 %u", "빠르게 두드려!",
    "점수 %u", "정말 즐거웠어!", "+기분", "시간 맞추기", "시", "분", "B: 취소", "언어", "메달 획득!", "멋진데!",
    "%u일 연속!", "연속 %u  최고 %u", "유대", "나무열매 ???", "빨간 열매", "파란 열매", "초록 열매", "%s   %lu일째",
    "A: 이름 바꾸기", "전투", "공격", "방어", "속도", "무게", "힘 기르기", "메달 %d/%d", "B: 뒤로", "이름",
    "B: 뒤로", "포만", "기분", "기력", "청결", "최고 %u", "성장", "Lv.%u", "%u분 뒤 Lv.%u", "진화",
...
void setLang(Lang l) {
  ...
  p.end();
  /* Species names are a per-language table on the card, so the pointers in
   * DEX_TBL have to be repointed or the UI switches language and keeps the old
   * names. */
