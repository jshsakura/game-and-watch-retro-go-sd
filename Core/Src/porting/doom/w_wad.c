...
static void ExtendLumpInfo(int newnumlumps)
{
    ...
    /* GNW VENDORED COPY of doomgeneric w_wad.c. Only change: the lumpinfo[]
     * array (the WAD directory, ~64KB for DOOM1.WAD) is allocated from the
     * LUT8 bonus pool instead of calloc -> ram_malloc, so it does NOT eat into
     * the DOOM zone. Allocated once (one IWAD) and not freed during a session,
     * which suits the bonus pool's bump allocator. Rest is upstream-identical. */
    {
        extern void *doom_bonus_alloc(size_t n);
        newlumpinfo = (lumpinfo_t *)doom_bonus_alloc(
                          (size_t)newnumlumps * sizeof(lumpinfo_t));
        if (newlumpinfo != NULL)
            memset(newlumpinfo, 0, (size_t)newnumlumps * sizeof(lumpinfo_t));
    }
