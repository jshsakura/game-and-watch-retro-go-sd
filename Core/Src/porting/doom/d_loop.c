...
void TryRunTics (void)
{
	...
	{
	    { static int _m=0; if(!_m){_m=1; printf("[doom] >> TryRunTics: early return (escape to display)\n");} }
	    return;
	}
    ...
    { static int _m=0; if(!_m){_m=1; printf("[doom] >> TryRunTics: wait done, running tics\n");} }
    while (counts--)
    {
        ticcmd_set_t *set;
        ...
        if (!PlayersInGame())
        {
            { static int _m=0; if(!_m){_m=1; printf("[doom] >> TryRunTics: !PlayersInGame, return\n");} }
            return;
        }
	...
	for (i=0 ; i<ticdup ; i++)
	{
            ...
            { static int _m=0; if(!_m){_m=1; printf("[doom] >> calling RunTic (G_Ticker) gametic=%d\n", gametic);} }
            loop_interface->RunTic(set->cmds, set->ingame);
            { static int _m=0; if(!_m){_m=1; printf("[doom] >> RunTic returned\n");} }
	    gametic++;
