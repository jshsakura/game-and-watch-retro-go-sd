 ...
 * DEVICE FORK: the original 512 KB tape buffer is embedded by value inside c64_t,
 * which pushes sizeof(c64_t) to ~790 KB — over this MCU's ~724 KB RAM_EMU overlay.
 * This port's initial scope is .prg/.crt only (no .tap), and the datassette is left
 * disabled (desc.c1530_enabled = false), so the buffer is never used. Shrink it to
 * reclaim ~512 KB; restore the original value if .tap support is ever added. */
#define C1530_MAX_TAPE_SIZE (1024)
