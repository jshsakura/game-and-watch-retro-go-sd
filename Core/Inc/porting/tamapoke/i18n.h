...
// Supported languages. The Latin rows carry no accents because upstream's
// bitmap font had none. Korean does not go through that font at all: it is
// drawn with the launcher's own i18n renderer, which is resident firmware and
// already loads fonts/unicode_hangul.bin from the card, so it costs the core
// nothing in RAM_EMU.
